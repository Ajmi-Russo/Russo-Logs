...
CreateThread(function() --[[ Permissions check for buildin DiscordAcePerms ]]
	...
	if not IsPrincipalAceAllowed('resource.' .. GetCurrentResourceName(), 'command.add_principal') then
		print('^1Error:^0 russo_logs needs to have access to ^2add_principal^0 for the discord permissions to work.')
		add_principal = false
	end
	if not IsPrincipalAceAllowed('resource.' .. GetCurrentResourceName(), 'command.remove_principal') then
		print('^1Error:^0 russo_logs needs to have access to ^2remove_principal^0 for the discord permissions to work.')
		remove_principal = false
	end
	if not IsPrincipalAceAllowed('resource.' .. GetCurrentResourceName(), 'command.add_ace') then
		print('^1Error:^0 russo_logs needs to have access to ^2add_ace^0 for the discord permissions to work.')
		addd_ace = false
	end
	if not IsPrincipalAceAllowed('resource.' .. GetCurrentResourceName(), 'command.remove_ace') then
		print('^1Error:^0 russo_logs needs to have access to ^2remove_ace^0 for the discord permissions to work.')
		remove_ace = false
	end
	...
	if Config ~= nil then
		if not Config.token then
			print('^1Error: Issue loading config file. Please follow the installation guild on the docs: https://docs.russo.com^0')
		end
...
AddEventHandler("playerJoining", function(newID, oldID) --[[ Name Change Logs / Discord Ace Perms. ]]
	...
	local oldName = GetResourceKvpString("russo_logs:nameChane:"..ids.license)
	if oldName == nil then
		SetResourceKvp("russo_logs:nameChane:"..ids.license, GetPlayerName(newID))
	else
...
RegisterNetEvent("russo:russo_logsV3:GetConfigSettings")
AddEventHandler("russo:russo_logsV3:GetConfigSettings", function()
	if Config ~= nil then
		data = {
		...
		TriggerClientEvent("russo:russo_logsV3:SendConfigSettings", source, data)
	end
end)

RegisterServerEvent('russo:russo_logsV3:playerShotWeapon') --[[ Shooting logs. ]]
AddEventHandler('russo:russo_logsV3:playerShotWeapon', function(weapon, count)
	if not weapon then
...
RegisterServerEvent('russo:russo_logsV3:playerDied')
AddEventHandler('russo:russo_logsV3:playerDied',function(args)
	if args.kil == 0 then
...
RegisterServerEvent('russo:russo_logsV3:PlayerDamage') --[[ Damaghe Logs. ]]
AddEventHandler('russo:russo_logsV3:PlayerDamage', function(args)
...
RegisterServerEvent('russo:russo_logsV3:ClientDiscord')
AddEventHandler('russo:russo_logsV3:ClientDiscord', function(args)
...
RegisterNetEvent("russo:russo_logsV3:ScreenshotCB") --[[ Returning screenshot value. ]]
AddEventHandler("russo:russo_logsV3:ScreenshotCB", function(args)
	ServerFunc.CreateLog(args)
end)

CreateThread( function() --[[ Version Checker ]]
	local version = GetResourceMetadata(GetCurrentResourceName(), 'version')
	PerformHttpRequest('https://raw.githubusercontent.com/JohnnyS/russo_logsV3/master/json/version.json', function(code, res, headers)
		if code == 200 then
...
-------------------------------------------------------^0]]):format(rv.version, rv.changelog))
				ServerFunc.CreateLog({ EmbedMessage = "**russo_logsV3 Update V"..rv.version.."**\nDownload the latest update of russo_logsV3 here:\nhttps://github.com/JohnnyS/russo_logsV3/\n\n**Changelog:**\n"..rv.changelog..'\n\n**How to update?**\n1. Download the latest version.\n2. Replace all files with your old once **EXCEPT THE CONFIG** folder.\n3. run the `!russologs setup` command again and you\'re done.', channel = 'system'})
			end
		end
	end, 'GET')
end)
...
exports.russo_logsV3:createLog({
