...
- delete
	- !russologs delete carrot
		- Will delete the channel carrot from the channels.json and the linked channel on discord.
- hide
	- !russologs hide screenshot ip
		- Will hide the ip from the screenshot logs.
- resethook
	- !russologs resethook
		- Will create a new webhook for imageStore. (Everytime russo_logsV3 starts the resource will create a new webhook anyways.)
- players
	- !russologs players
		- Will return a list of online players and their server id.
- screenshot
	- !russologs screenshot 1
...
- uninstall
	- !russologs uninstall
		- Will remove all channels created by russo_logsV3 from the discord.
# Installation
...
2. Put the russo_logsV3 folder in the server resource directory
    -   Make sure to rename the folder to  **russo_logsV3**.
...
3. Get yourself the bot token and add them in the  `config.json`
    -   Not sure how to get a bot token?  [How to get a bot token.](https://forum.russo.com/d/12-how-to-get-a-discord-bot-token)
...
6.  Go to your discord where you invited the bot (_The one where you want your new main logs to be._) and use the command  `!russologs setup`.
7.  Restart your server and you will see the logs on your discord.
  
 # Configuration

After you have installed russo_logs you can open the config file and change the settings to your needs. You will find the config file in the config folder.

When you just installed russo_logs your config should look like this:
...
```json
{
    ...
    "NameChangePerms": "russo.staff",
    "screenshotPerms": "russo.staff",
...
To create custom logs you will need to add the export to the event/function or command you want to log. This is in the resource you want to log You can use the command  `!russologs create`  on your discord server to setup a custom logs channel.
...
-   **channel:**  Will be pre filled out if you use the  `!russologs create`  command.
