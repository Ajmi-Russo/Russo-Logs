module.exports = {
	...
	async execute(message) {
		...
		if(content.toLowerCase().startsWith(`${client.config.prefix}hide`)){
			...
			let status = await GetResourceKvpString(`russo_logs:${(args[2])}:${args[3]}`)
			if(status === "false" || status === null){
				status = 'true'
			} else {
				status = 'false'
			}
			await SetResourceKvp(`russo_logs:${args[2].toLowerCase()}:${args[3].toLowerCase()}`, status)
...
\`${client.config.prefix}setup\` - Run the russo_logsV3 Setup.
\`${client.config.prefix}uninstall\` - Remove all channels used in russo_logsV3.
