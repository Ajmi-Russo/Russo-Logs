module.exports = {
	...
	async execute(message) {
		...
		if(content.toLowerCase().startsWith(`${client.config.prefix}setup`)){
			...
			await channel.send('**Starting russo_logs Setup!**\n*ETA: 30 seconds*').then(msg => {
				response = msg
			})
			ids = ''
			info = {
			...
			if(x){
				if(!guild.channels.cache.find(cc => cc.name === `📢・russo-announcements`)){
					await guild.channels.create("📢・russo-announcements", {
							type: 'GUILD_TEXT',
			...
			setTimeout(() => {
				response.edit(`**Starting russo_logs Setup!**\n*ETA: 15 seconds*`)
			}, 15000)
			setTimeout(() => {
